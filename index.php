...
<script async src="https://www.googletagmanager.com/gtag/js?id=UA-153680849-1"></script>
<script>
  window.dataLayer = window.dataLayer || [];
  function gtag(){dataLayer.push(arguments);}
  gtag('js', new Date());

  gtag('config', 'UA-153680849-1');
</script>


  <meta charset="utf-8">
  <meta name="google-site-verification" content="5iy7ukMmvrSFJZ8xtwTUIW_BxI4nKJHPtzFIKCxSbUg" />
  ...
  <section>
              ...
              <ul class="list-block list-li-ptb-15 list-btm-border-white bg-primary text-center">
						...
						<?php
							...
							$sel_post = mysqli_query($con,"SELECT * FROM `blog` ORDER BY `blog_date` DESC LIMIT $limit") or die('ErrorD');
								...
								$category = 'blog';
                $sel_viewn = mysqli_query($con, "SELECT * FROM `like_view` WHERE post_id='$blog_id' AND category='$category'");
                    $view=0;
                  while($row = mysqli_fetch_array($sel_viewn)) {
                    $view = $row['view'];
                }
                $sel_comments = mysqli_query($con, "SELECT * FROM `comments` WHERE post_id='$blog_id' AND category='$category'");
                $get_total = mysqli_num_rows($sel_comments);
							...
							<ul class="list-li-mr-20 pt-10 pb-20">
								...
								'.$date.'</li>
								<li><i class="color-primary mr-5 font-12 ion-ios-bolt"></i><b>'.$view.'</b></li>
								<li><i class="color-primary mr-5 font-12 ion-chatbubbles"></i><b>'.$get_total.'</b></li>
							</ul>
							<p></p>';
								}
							?>
		...
		<!-- Tech news starts -->
            ...
            <div class="popular-news-widget mb-30">
                <h4 style="color: red;">Recent Technology News</h4>
                <?php $blog->view_technews(); ?>
            </div>
            <!-- Tech News-->
