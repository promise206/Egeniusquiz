...
$result = mysqli_query($con,"SELECT * FROM `jambquiz` ORDER BY RAND()") or die('Error');
...
if($rowcount == 0){
	...
	<td><b><a href="jambaccount.php?q=jambquiz&step=2&eid='.$eid.'&n=1&t='.$total.'" class="pull-right btn sub1" style="margin:0px;background:#2b5f05"><span class="glyphicon glyphicon-new-window" aria-hidden="true"></span>&nbsp;<span class="title1"><b>Start</b></span></a></b></td></tr>';
}
else
{
...
//Time Starts
if (isset($_SESSION['jambtime'])) {
  $dateFormat = "d F Y -- g:i a";
    // session variable_exists, use that
    $targetDate = $_SESSION['jambtime'];
} else {
    // No session variable, red from mysql
    $result=mysqli_query($con,"SELECT * FROM `jambquiz` WHERE `eid`='$eid' LIMIT 1");
    $time=mysqli_fetch_array($result);
    $dateFormat = "d F Y -- g:i a";
    $targetDate = time() + ($time['jambtime']*60);
    $_SESSION['jambtime'] = $targetDate;
}
...
echo '<form action="update.php?q=jambquiz&step=2&eid='.$eid.'&n='.$sn.'&t='.$total.'&qid='.$qid.'" method="POST"  class="form-horizontal">
<br />';
$i=1;
while($row=mysqli_fetch_array($q) )
{
...
<script type="text/javascript">
      ...
      function setCountDown ()
      {
          ...
          if (minutes == '00' && seconds == '00') { 
              ...
              window.location = "update.php?q=jambquiz&step=7&eid=<?php echo $eid; ?>&n=<?php echo $sn; ?>&t=<?php echo $total; ?>&qid=<?php echo $qid; ?>&ans=''"
...
<?php }
  ...
  echo "<script>window.open('jambquiz_start.php','_self')</script>";
}
?>