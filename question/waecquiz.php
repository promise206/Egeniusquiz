...
<html lang="en">
<head>
  <title>Waecquiz Reg - Egeniusquiz</title>
  ...
  <!--/ Banner ends-->
	...
	<section class="ptb-0">
		...
		<div class="container">
			...
			<a class="color-ash mt-10" href="waecquiz.php">WAEC QUIZ</a>
		</div><!-- container -->
	</section>
...
<h2 align="center" style="font-family:'typo'; color:#000066"><br><br>REGISTER FOR WAEC MONTHLY PREPARATORY QUIZ</h2>
<div style="font-size:14px">
<?php if(@$_GET['q'])echo '<span style="font-size:18px;"><span class="glyphicon glyphicon-ok" aria-hidden="true"></span>&nbsp;'.@$_GET['q'].'</span>';
else
{echo' 
  <p style="color:green;"><b>Only subscribed members can partake in the quiz.</b></p><br />
  <div class="row">
  <div class="col-md-12"></div>
  <div class="col-md-10">
  </div><div class="col-md-1"></div></div>
  <p style="color:black;"><b> WAEC Preparatory quiz registration starts on 5th November 2019 - 6th March 2020 and the quiz will holds on 7th March 2020.
  ...
  <form role="form"  method="post" action="waecquiz.php">
...
<div class=" col-md-6">
  ...
  <div class="col-md-6">
    ...
    <select name="sub4" id="sub4" class="form-control">
      ...
      <div class="col-md-6">
      <label for="pid" class="control-label">Proffessional Code:</label>
        <input type="text" class="form-control" id="pid" name="pid" placeholder="Enter Code Sent to Your Email" required>
      </div>
  </div>
...
if(isset($_POST['submit']))
{
  ...
  if($sub1==$sub2||$sub1==$sub3||$sub1==$sub4||$sub2==$sub3||$sub2==$sub4||$sub3==$sub4){
    echo "<script>alert('You Choosed Same Subject Twice!')</script>";
    echo "<script>window.open('waecquiz.php','_self')</script>";
    exit();
  }
  ...
  $sel_status = "SELECT * FROM profquiz WHERE pid = '$pid' AND reg_status='$reg_status' ";
  ...
  $sel_pid = "select * from profquiz where pid = '$pid' ";
  $run_pid = mysqli_query($con, $sel_pid);
  $check_pid = mysqli_num_rows($run_pid);
  if($check_pid>0){
    $q=mysqli_query($con,"UPDATE `profquiz` SET `firstname`='$firstname',`lastname`='$lastname',`email`='$email',`time`='$time',`date`='$date',`sub1`='$sub1',`sub2`='$sub2',`sub3`='$sub3',`sub4`='$sub4',`reg_status`='$reg_status', `active`='$active' WHERE `pid`='$pid'")or die('Error147');
  if($q)
  {
