...
class Comments {
  ...
  /* [COMMENTS FUNCTIONS] */
  function get($pid = 0,$category = 0) {
    // get() : get all comments for the given post
    // PARAM $pid : post ID
    
    $sql = "SELECT * FROM `comments` WHERE `post_id`=? AND `category`='$category' ORDER BY `reply_id`";
    $this->stmt = $this->pdo->prepare($sql);
  ...
  function BadWordFilter($text, $replace, $bads)
	{	
  ...
  function add($pid, $first_name, $name, $message, $rid, $category) {
  // add() : add new comment
  // PARAM $pid : post ID
  //       $name : name
  ...
  //       $rid : reply id
    ...
    $fields = "`post_id`, `first_name`, `name`, `message`, `category`";
    $values = "?, ?, ?, ?, ?";

    $commentprev = strlen($message);
		if($this->badword){
		$message = $this->BadWordFilter($message,1,$this->badword); 
		$commentnext = strlen($message);

    // Clean out HTML tags, prevent XSS
    $message = str_replace("<", "&lt;", $message);
    $message = str_replace(">", "&gt;", $message);

    $cond = [$pid, $first_name, $name, $message, $category];
    if (is_numeric($rid)) {
      $fields .= ", `reply_id`";
      $values .= ", ?";
      $cond[] = $rid;
